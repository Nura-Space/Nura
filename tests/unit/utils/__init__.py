...
class TestImageProcessor:
    ...
    @pytest.mark.unit
    async def test_process_large_image_resizes(self, large_image_bytes, caplog):
        """Test that large images are resized."""
        result = await ImageProcessor.process(large_image_bytes)
        assert result is not None
        assert isinstance(result, str)
        # Check that resizing happened
        assert "Resized image from 2000x1500" in caplog.text
