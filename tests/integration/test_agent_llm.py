...
class TestAgentLLMIntegration:
    ...
    async def test_toolcall_agent_with_mock_llm(self, mock_llm, mock_tool_collection, mock_sandbox):
    ...
    async def test_toolcall_agent_with_mock_tool_response(self, mock_llm, mock_tool_collection, mock_sandbox):
    ...
    async def test_agent_state_transitions(self, mock_llm, mock_tool_collection, mock_sandbox):
    ...
    async def test_agent_state_context_error_handling(self, mock_llm, mock_tool_collection, mock_sandbox):
    ...
    async def test_agent_memory_updates(self, mock_llm, mock_tool_collection, mock_sandbox):
    ...
    async def test_agent_stuck_state_detection(self, mock_llm, mock_tool_collection, mock_sandbox):
    ...
    async def test_agent_not_stuck_with_different_responses(self, mock_llm, mock_tool_collection, mock_sandbox):
...
class TestAgentWithRealLLM:
    ...
    async def test_agent_token_tracking(self, real_llm):
        ...
        with patch("nura.agent.base.SANDBOX_CLIENT", mock_sandbox):
            ...
            assert real_llm.total_input_tokens > 0 or real_llm.total_completion_tokens > 0
