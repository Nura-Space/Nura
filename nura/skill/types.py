...
class Skill(BaseModel):
    ...
    source: str = Field(
        default="builtin", description="Source of the skill: builtin or workspace"
    )

    # Additional fields for progressive disclosure
    requires: Optional[SkillRequires] = Field(
        default=None, description="Skill requirements"
    )
    always: bool = Field(default=False, description="Whether to always load this skill")
    available: bool = Field(
        default=True, description="Whether the skill is available (requirements met)"
    )
    blocking: bool = Field(
        default=False, description="Whether to block execution and wait for result"
    )
