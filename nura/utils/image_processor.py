...
class ImageProcessor:
    ...
    async def process(image_bytes: bytes) -> Optional[str]:
        ...
        """
        return ImageProcessor.process_sync(image_bytes)

    @staticmethod
    def process_sync(image_bytes: bytes) -> Optional[str]:
        """Process an image synchronously (resize and encode to base64).
        ...
        try:
            ...
            # Convert to RGB if not already (RGBA can't be saved as JPEG)
            if img.mode == "RGBA":
